...
#_(defn canvas-progress-bar [{:keys [progress width height]}]
  {:fx/type :canvas
   :width width
   :height height
   ...
   :draw (fn [^Canvas canvas]
           (binding [*ctx* (.getGraphicsContext2D canvas)]
             ...
             (draw (apply  #'todo/todo-app
                           :selected-filter :all
                           (sequence cat @todo/todo-state))))
...
(defn membrane->cljfx [set-state component]
  )



#_(defmacro membrane-component [ui-var state set-state]
  (let [
        handler (fn [& args]
                  "ASdf")
                  ...
                  ([type & args]
                   (case type
                     ...
                     (let [[path v] args]
                       ...
                       #_(if #{'[[(keypath :membrane.component/context)] :focus]
                               '[[(keypath :membrane.component/context) (keypath :focus)]]}
      ...
      {:fx/type :stage
       ...
       :scene {:fx/type :scene
               :root {:fx/type :v-box
                      :padding 100
                      :spacing 50
                      :children [#_{:fx/type canvas-progress-bar
                                    :width 300
                                    :height 300
                                    :progress progress}
                                 (membrane-component #'todo/todo-app
                                                     (:todo-state all-state)
                                                     #(swap! *progress update :todo-state %))
                                 {:fx/type :slider
                                  :pref-width 100
                                  :min 0
                                  :max 1
                                  :value (:progress all-state)
                                  :on-value-changed #(swap! *progress assoc :progress %)}]}}}))))
...
(extend-type membrane.ui.RoundedRectangle
  ...
  (draw [this]
    (case *paint-style*
      :membrane.ui/style-fill (.fillRoundRect ^GraphicsContext *ctx*
                                              0 0
                                              (:width this) (:height this)
                                              (:border-radius this) (:border-radius this))
      :membrane.ui/style-stroke (.strokeRoundRect ^GraphicsContext *ctx*
                                                  0 0
                                                  (:width this) (:height this)
                                                  (:border-radius this) (:border-radius this))
      :membrane.ui/style-stroke-and-fill (do (.strokeRoundRect ^GraphicsContext *ctx*
                                                               0 0
                                                               (:width this) (:height this)
  ...
  (let [padding (float 5)]
      (if *already-drawing*
        ...
        (let [[xscale yscale :as content-scale] [2 2] ;; @(:window-content-scale *window*)
          ...
          ;;(.drawImageRect ^Canvas *ctx* img (Rect/makeWH img-width img-height) (map->paint *paint*) )
          ))))
...
;; com.sun.javafx.scene.text.TextLayout/TYPE_TEXT
;; (def my-font (com.sun.javafx.scene.text.FontHelper/getNativeFont (get-java-font ui/default-font)))
;; (.setContent text-layout "hello" my-font)

;; com.sun.javafx.text.TextLayout/
;; (.getRange text-layout 0 1 com.sun.javafx.scene.text.TextLayout/TYPE_TEXT 0 0)




;; toolkit.getToolkit().getTextLayoutFactory().createLayout()
;; https://java-browser.yawk.at/org.openjfx/javafx-controls/11/com/sun/javafx/scene/control/skin/Utils.java#149
...
(defn printable? [c]
  (let [block (java.lang.Character$UnicodeBlock/of \backspace)]
    (and (not (Character/isISOControl ^char c))
         (not= KeyEvent/CHAR_UNDEFINED c)
         (some? block)
         (not= block java.lang.Character$UnicodeBlock/SPECIALS))))


#_(defn- -key-typed [window e]
  (let [c (.getKeyChar e)
        ui @(:ui window)]
    (try
      (when (printable? c)
...
#_(defn make-uber-listener [window]
...
(defn -main [& args]
  #_(run ((requiring-resolve 'membrane.component/make-app)
